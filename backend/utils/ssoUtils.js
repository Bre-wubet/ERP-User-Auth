...
const SSO_ISSUER = process.env.SSO_ISSUER || 'erp-system';

export const generateSAMLRequest = (config) => {
  const {
    issuer = SSO_ISSUER,
    destination,
...
export const generateSAMLResponse = (config, user) => {
  const {
    issuer = SSO_ISSUER,
    destination,
    assertionConsumerServiceURL,
...
export const generateOAuth2AuthURL = (config) => {
  const {
    clientId,
    redirectUri,
...
export const exchangeOAuth2Code = async (config, code) => {
  const {
    clientId,
    clientSecret,
    redirectUri,
...
export const getOAuth2UserInfo = async (config, accessToken) => {
...
export const generateSSOToken = (user, config = {}) => {
  const {
    expiresIn = '1h',
    issuer = SSO_ISSUER,
    audience = 'erp-system'
...
export const verifySSOToken = (token, config = {}) => {
  const {
    issuer = SSO_ISSUER,
    audience = 'erp-system'
  } = config;
...
export const generateState = (data = {}) => {
  const state = {
...
export const verifyState = (state, maxAge = 10 * 60 * 1000) => {
  try {
